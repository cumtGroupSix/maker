...
<script>
    export default {
        ...
        axiosMakerRegisterInfo(){
        	this.axios.post("/api/user/makerSignUpInfo?username="+this.username+"&storeName="+this.storename+"&mobileNumber="+this.mobilenumber+"&email="+this.email+"&school="+this.school+"&realName="+this.realname+"&studentId="+this.studentid)
        	.then((response)=>{if(response.status==200&&response.data==1){
                localStorage.setItem('makerusername',this.username);
                localStorage.setItem('makerpassword',this.password);
                this.$Message.success('注册成功，请登录');
                this.$router.push({path: '/makerlogin'});                
        	}else{
